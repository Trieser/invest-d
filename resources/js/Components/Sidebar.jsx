...
function GenrePopout({ genres, currentGenreId, onGenreClick, onMouseEnter, onMouseLeave, mobile }) {
    return (
        <div
            onMouseEnter={onMouseEnter}
            onMouseLeave={onMouseLeave}
            className={`absolute z-50 bg-white rounded-xl shadow-2xl border border-green-100 p-2 ${mobile ? 'left-1/2 -translate-x-1/2 top-full mt-2 min-w-[180px] max-w-[240px]' : 'left-full top-0 ml-2 min-w-[180px] max-w-[240px]'} max-h-80 overflow-y-auto`}
        ...
        >
            {genres.map(item => (
                <button
                    ...
                    className={`flex items-center gap-2 w-full px-2 py-1 rounded-lg mb-1 last:mb-0 text-left transition-colors ${item.active ? 'bg-blue-100 text-green-700 font-bold' : 'text-gray-700 hover:bg-green-50'}`}
                >
                    <span className="text-xl">{item.icon}</span>
                    <span className="text-sm whitespace-normal break-words max-w-[160px]">{item.label}</span>
                    {item.badge !== undefined && (
                        <span className="ml-auto bg-blue-400 text-white text-xs font-bold px-1.5 py-0.5 rounded-full min-w-[18px] h-[18px] flex items-center justify-center">{item.badge}</span>
                    )}
                </button>
            ))}
        </div>
...
export default function Sidebar({ auth, currentPage = 'photos', currentGenreId = null, activeGenres = [] }) {
    ...
    const menu = [
        { icon: '🏠', page: 'dashboard', onClick: () => Inertia.visit('/user/dashboard'), label: '' },
        { icon: '➕', page: 'photos', onClick: () => Inertia.visit('/user/photos'), label: 'Add Photo' },
        { icon: '🏷️', page: 'genres', onClick: () => Inertia.visit('/user/genres'), label: 'All Genres' },
    ];
    ...
    const handleYourPhotoLeave = () => {
        popoutTimeout.current = setTimeout(() => setShowGenrePopout(false), 120);
    };
        ...
        >
            ...
            <div className="flex flex-1 sm:flex-col flex-row items-center justify-between w-full h-full gap-2 px-2 sm:px-0">
                ...
                <div className="relative flex flex-col items-center sm:my-0 my-1">
                    <button
                        className={`flex flex-col items-center focus:outline-none transition-all duration-200 hover:scale-110 hover:shadow-lg hover:bg-green-50 rounded-xl py-1 px-2 ${currentPage.startsWith('genre-') ? 'font-bold text-green-700' : 'text-gray-700'}`}
                    ...
                    >
                        ...
                        <span className="text-xs mt-0.5">{yourPhotoBtn.label}</span>
                    </button>
...
function SidebarIcon({ icon, label, active, onClick, mobile }) {
    return (
        <button
            onClick={onClick}
            className={`flex flex-col items-center focus:outline-none transition-all duration-200 hover:scale-110 hover:shadow-lg hover:bg-green-50 rounded-xl py-1 px-2 ${active ? 'font-bold text-green-700' : 'text-gray-700'} ${mobile ? 'text-xs' : ''}`}
            style={{wordBreak: 'break-word'}}
        >
            <span className="text-2xl relative">{icon}</span>
            <span className="text-xs mt-0.5 text-center whitespace-normal break-words max-w-[64px]">{label}</span>
        </button>
    );
} 